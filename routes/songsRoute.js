...
router.post('/add-playlist', authmiddleware, async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.body.userId, { playlists: existingPlaylists },
            { new: true });

        res.status(200).send({ message: 'Playlist created successfully', success: true, data: updatedUser, });

    } catch (error) {
        res.status(500).send({ message: 'Error creating playlist', success: false, data: error });
    }
});
...
router.post('/update-playlist', authmiddleware, async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message: 'Playlist updated successfully', success: true, data: updatedUser, });

    } catch (error) {
        res.status(500).send({ message: 'Error updating playlist', success: false, data: error });
    }
});
...
router.post('/delete-playlist', authmiddleware, async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message: 'Playlist deleted successfully', success: true, data: updatedUser, });

    } catch (error) {
        res.status(500).send({ message: 'Error deleting playlist', success: false, data: error });
    }
});
